...
def run_spec(prompt: str, target_dir_path: str | None) -> int:
    ...
    if not template_file.is_file():
        print(
            f"Error: spec template not found at '{template_file}'. Run `./bin/coding-spec init` first.",
            file=sys.stderr,
        )
        return 1

    content = template_file.read_text(encoding="utf-8").replace("[Feature Name]", prompt)
